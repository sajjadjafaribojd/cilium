				     ...
				     bool has_l4_header)
{
	...
	return snat_v4_rewrite_headers(ctx, IPPROTO_ICMP, ETH_HLEN, has_l4_header, (__u32)off,
					 ...
					 struct ipv4_nat_entry **state)
{
	...
	*state = snat_v4_lookup(&tuple);
	...
	return snat_v4_rewrite_headers(ctx, tuple.nexthdr, (__u32)inner_l3_off, true, icmpoff,
		...
		struct trace_ctx *trace, __s8 *ext_err __maybe_unused)
{
	...
	case IPPROTO_SCTP:
#endif  /* ENABLE_SCTP */
		ret = ipv4_load_l4_ports(ctx, ip4, (__u32)off, CT_INGRESS,
					 &tuple.dport, &has_l4_header);
		if (ret < 0)
			return ret;
...
rewrite:
	return snat_v4_rewrite_headers(ctx, tuple.nexthdr, ETH_HLEN, has_l4_header, (__u32)off,
